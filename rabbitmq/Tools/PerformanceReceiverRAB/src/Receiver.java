...
public class Receiver {
	...
	public static int WAIT_DELAY_MS;
	...
	public static void main(String[] args) {
		...
		try {
			...
			Receiver.WAIT_DELAY_MS = Integer.parseInt(props.getProperty("waitdelayms", "300"));
			Receiver.MSG_COUNT = Integer.parseInt(props.getProperty("msgcount"));
		...
		for(int i=0; i<100; i++) {
			...
			if(server != null) {
				...
				try {
					...
					ReceiverRunnable rec = new ReceiverRunnable(i, hostname, port);
					servers.add(rec);
				} catch(Exception e) {
					e.printStackTrace();
				}
	...
	private static void printEndReport() {
		long througput = (Receiver.MSG_COUNT*1000/(allEnd - allStart));
		
		System.out.println("----------------------------------------------------------------------");
		System.out.println("FINAL REPORT :");
		System.out.println(througput + " msg/sec");
		System.out.println(Receiver.MSG_COUNT + " messages");
		System.out.println("----------------------------------------------------------------------");
